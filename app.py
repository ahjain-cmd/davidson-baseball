...
def _get_opp_pitcher_profile(tm, pitcher_name, team):
    """Extract opponent pitcher profile from TrueMedia."""
    ...
    profile = {
        ...
        "whip": _safe_num(trad, "WHIP"),
        "velo": _safe_num(mov, "Vel"),
        "chase_pct": _safe_num(pr, "Chase%"),
        "swstrk_pct": _safe_num(pr, "SwStrk%"),
        "ev_against": _safe_num(exit_d, "ExitVel"),
        "barrel_pct": _safe_num(exit_d, "Barrel%"),
...
def _get_our_hitter_profile(data, batter_name, season_filter=None):
    """Extract Davidson hitter profile from Trackman data."""
...
def _score_pitcher_vs_hitter(arsenal, hitter_profile):
    ...
    return {
        ...
        "hitter_data": {
            ...
            "chase_pct": hitter_profile.get("chase_pct", np.nan), "whiff_pct": hitter_profile.get("swstrk_pct", np.nan),
            "ev": hitter_profile.get("ev", np.nan), "barrel_pct": hitter_profile.get("barrel_pct", np.nan),
            "gb_pct": hitter_profile.get("gb_pct", np.nan), "pull_pct": hitter_profile.get("pull_pct", np.nan),
...
def _score_hitter_vs_pitcher(hitter_tm, pitcher_profile):
    ...
    for opp_pitch, opp_usage in pitcher_profile["pitch_mix"].items():
        ...
        if our_data:
            ...
            if not pd.isna(our_whiff):
                score -= (our_whiff - 20) * 0.3
                if our_whiff > 35:
                    notes.append(f"vulnerable ({our_whiff:.0f}% whiff)")
...
def _pitching_plan_content(tm, team, data, season_filter):
    ...
    # ── Scouting Cards — data-first, compact ──
    st.markdown("---")
    for matchup in all_matchups:
        ...
        best_pt = max(ps.items(), key=lambda x: x[1]["score"]) if ps else None
        # Header with key stats inline
        label_parts = [f"{display_name(matchup['hitter'])} ({matchup['bats']})"]
        label_parts.append(f"Score: {score:.0f}")
        label_parts.append(matchup["platoon"])
        ops_str = f"{hd.get('ops', 0):.3f}" if not pd.isna(hd.get("ops", np.nan)) else "?"
        k_str = f"{hd.get('k_pct', 0):.0f}%" if not pd.isna(hd.get("k_pct", np.nan)) else "?"
        ch_str = f"{hd.get('chase_pct', 0):.0f}%" if not pd.isna(hd.get("chase_pct", np.nan)) else "?"
        label_parts.append(f"OPS {ops_str} | K {k_str} | Chase {ch_str}")
        with st.expander(" | ".join(label_parts)):
            # Row 1: Hitter profile metrics
            hd_cols = st.columns(6)
            _metrics = [
                ("wOBA Split", f"{hd.get('woba_split', 0):.3f}" if not pd.isna(hd.get("woba_split", np.nan)) else "-"),
                ("EV", f"{hd.get('ev', 0):.1f}" if not pd.isna(hd.get("ev", np.nan)) else "-"),
                ("Barrel%", f"{hd.get('barrel_pct', 0):.1f}" if not pd.isna(hd.get("barrel_pct", np.nan)) else "-"),
                ("GB%", f"{hd.get('gb_pct', 0):.0f}" if not pd.isna(hd.get("gb_pct", np.nan)) else "-"),
                ("BB%", f"{hd.get('bb_pct', 0):.0f}" if not pd.isna(hd.get("bb_pct", np.nan)) else "-"),
                ("Pull%", f"{hd.get('pull_pct', 0):.0f}" if not pd.isna(hd.get("pull_pct", np.nan)) else "-"),
            ]
            for j, (lbl, val) in enumerate(_metrics):
                with hd_cols[j]:
                    st.metric(lbl, val)
            # Row 2: Per-pitch matchup table
            rows = []
            for pt_name, pt_data in sorted(ps.items(), key=lambda x: x[1]["score"], reverse=True):
                # Find best tunnel pair involving this pitch
                tun_info = "-"
                if not isinstance(tunnels, pd.DataFrame) or not tunnels.empty:
                    t_match = tunnels[(tunnels["Pitch A"] == pt_name) | (tunnels["Pitch B"] == pt_name)]
                    if not t_match.empty:
                        best_t = t_match.iloc[0]
                        partner = best_t["Pitch B"] if best_t["Pitch A"] == pt_name else best_t["Pitch A"]
                        tun_info = f"{best_t['Grade']} w/ {partner}"
                rows.append({
                    "Pitch": pt_name,
                    "Score": pt_data["score"],
                    "Whiff%": f"{pt_data['our_whiff']:.0f}" if not pd.isna(pt_data.get("our_whiff", np.nan)) else "-",
                    "CSW%": f"{pt_data['our_csw']:.0f}" if not pd.isna(pt_data.get("our_csw", np.nan)) else "-",
                    "Chase%": f"{pt_data['our_chase']:.0f}" if not pd.isna(pt_data.get("our_chase", np.nan)) else "-",
                    "EV Ag.": f"{pt_data['our_ev_against']:.1f}" if not pd.isna(pt_data.get("our_ev_against", np.nan)) else "-",
                    "S+": f"{pt_data['stuff_plus']:.0f}" if not pd.isna(pt_data.get("stuff_plus", np.nan)) else "-",
                    "Tunnel": tun_info,
                })
            if rows:
                st.dataframe(pd.DataFrame(rows), use_container_width=True, hide_index=True)
            # Row 3: Sequence plan — compact, data-driven
            sorted_ps = sorted(ps.items(), key=lambda x: x[1]["score"], reverse=True)
            if len(sorted_ps) >= 2:
                lead = sorted_ps[0][0]
                putaway_candidates = [(n, d) for n, d in sorted_ps if n != lead]
                if putaway_candidates:
                    putaway = putaway_candidates[0][0]
                    # Get sequence whiff% if available
                    seq_whiff = "-"
                    if not isinstance(sequences, pd.DataFrame) or not sequences.empty:
                        s_match = sequences[
                            (sequences["Setup Pitch"] == lead) & (sequences["Follow Pitch"] == putaway)
                        ]
                        if not s_match.empty:
                            seq_whiff = f"{s_match.iloc[0]['Whiff%']:.0f}%"
                    # Get tunnel grade for this pair
                    pair_tunnel = "-"
                    if not isinstance(tunnels, pd.DataFrame) or not tunnels.empty:
                        t_match = tunnels[
                        ...
                        if not t_match.empty:
                            pair_tunnel = f"{t_match.iloc[0]['Grade']} ({t_match.iloc[0]['Tunnel Score']:.0f})"
                    st.markdown(
                        f"**Sequence:** {lead} → {putaway} &nbsp;|&nbsp; "
                        f"Seq Whiff: {seq_whiff} &nbsp;|&nbsp; "
                        f"Tunnel: {pair_tunnel}"
                    )


def _hitting_plan_content(tm, team, data, season_filter):
    """Our Hitting Plan: how to attack their pitching staff."""
    ...
    with col_stats:
        era = opp_profile["era"]
        velo = opp_profile["velo"]
        whip = opp_profile["whip"]
        st.metric("ERA", f"{era:.2f}" if not pd.isna(era) else "?")
        st.metric("Velo", f"{velo:.1f}" if not pd.isna(velo) else "?")
        st.metric("WHIP", f"{whip:.2f}" if not pd.isna(whip) else "?")
    # Quick attack notes
    attack_notes = []
    ...
    for i, m in enumerate(all_matchups):
        ...
        summary_rows.append({
            ...
            "Weakness": f"{worst_pt[0]} ({worst_pt[1]['score']:.0f})",
            "Approach": m["approach_notes"][0] if m["approach_notes"] else "-",
        })
    st.dataframe(pd.DataFrame(summary_rows), use_container_width=True, hide_index=True)
    # Top 9 lineup cards
    ...
    for row_start in range(0, len(top9), 3):
        ...
        for j, col in enumerate(cols):
            ...
            with col:
                ...
                note = m["approach_notes"][0] if m["approach_notes"] else ""
                st.markdown(
                    ...
                    f'<div style="font-size:11px;color:#333;margin-top:4px;">{note}</div>'
                    f'</div>', unsafe_allow_html=True)
    # Individual approach cards
    st.markdown("---")
    section_header("Individual Approach Cards")
    for m in all_matchups:
        with st.expander(f"{display_name(m['hitter'])} ({m['bats']}) — Score: {m['overall_score']:.0f} | {m['platoon']}"):
            for note in m["approach_notes"]:
                st.markdown(f"- {note}")
            rows = []
...
def _scouting_pitcher_report(tm, team, trackman_data):
    ...
    if not pt.empty:
        ...
        for col, name in [("4Seam%", "fastball"), ("Sink2Seam%", "sinker"), ("Cutter%", "cutter"),
            ...
            if v is not None and not pd.isna(v) and v > primary_pct:
                ...
                primary_name = name
        if primary_pct > 50:
            notes.append(f"Relies heavily on {primary_name} ({primary_pct:.0f}%) — sit on it, time it, punish it.")
        # Count pitches in arsenal
        n_pitches = sum(1 for col in ["4Seam%", "Sink2Seam%", "Cutter%", "Slider%", "Curve%", "Change%", "Sweeper%"]
                        if not pd.isna(pt.iloc[0].get(col)) and pt.iloc[0].get(col, 0) > 5)
        if n_pitches <= 2:
            notes.append(f"Limited arsenal ({n_pitches} pitches) — narrows what to look for.")
        elif n_pitches >= 5:
            notes.append(f"Deep arsenal ({n_pitches} pitches) — be disciplined, don't chase unfamiliar looks.")
    ...
    if not pr.empty:
        ...
        if not pd.isna(comploc) and comploc < 40:
            notes.append(f"Poor command location ({comploc:.1f}% CompLoc) — be patient, he'll miss over the plate.")
        if not pd.isna(swstrk) and swstrk > 13:
            notes.append(f"Elite swing-and-miss ({swstrk:.1f}% SwStrk) — shorten up with 2 strikes, protect the plate.")
        elif not pd.isna(swstrk) and swstrk < 7:
            notes.append(f"Low swing-and-miss ({swstrk:.1f}% SwStrk) — this staff can be hit. Be aggressive.")
    ...
    if not mov.empty:
        ...
        if not pd.isna(eff_vel) and eff_vel > 93:
            notes.append(f"High effective velocity ({eff_vel:.1f} mph) — shorten swing, focus on timing.")
        elif not pd.isna(eff_vel) and eff_vel < 86:
            notes.append(f"Low effective velocity ({eff_vel:.1f} mph) — can afford to wait back, drive the ball.")
        if not pd.isna(ext) and ext > 6.5:
            notes.append(f"Long extension ({ext:.1f} ft) — ball gets on you fast, be ready early.")

    # LOB% / clutch
    if not rate.empty:
        lob = _safe_num(rate, "LOB%")
        if not pd.isna(lob) and lob < 65:
            notes.append(f"Low LOB% ({lob:.1f}%) — doesn't strand runners. Rally against him and keep the line moving.")
        elif not pd.isna(lob) and lob > 80:
            notes.append(f"High LOB% ({lob:.1f}%) — tough in the clutch. Need to score in bunches when you break through.")
